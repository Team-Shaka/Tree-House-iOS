...
extension Config {
    ...
    static let loginKey: String = {
        guard let key = Config.infoDictionary[Keys.Plist.loginKey] as? String else {
            fatalError("🎄⛔️Login_Key is not set in plist for this configuration⛔️🎄")
        }
        return key
    }()
}
